...
# find_claude_dir <cwd> <sub>: the first <dir>/.claude/<sub> walking up from cwd.
# The cwd is not the session root: in a multi-repo workspace it moves into a
# repo as soon as anything cds there. $HOME is the last directory checked, so
# ~/.claude/<sub> is a global fallback and nothing above it is ever read.
find_claude_dir() {
  local dir="$1"
  case "$dir" in /*) ;; *) return 0 ;; esac          # dirname of a relative path never reaches /
  while [ "$dir" != "/" ]; do
    [ -d "$dir/.claude/$2" ] && { echo "$dir/.claude/$2"; return 0; }
    [ "$dir" = "$HOME" ] && return 0
    dir=$(dirname "$dir")
  done
}
